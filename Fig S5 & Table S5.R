...
ggplot(field_mono_mean, aes(x = abbrev_focal, y = mono_mean)) + 
  ...
  annotate("text", x = 5.2, y = 0.50, parse = TRUE, size = 3.5, label = expression("W" ~ ":" ~ italic(p) == 0.008)) + 
  annotate("text", x = 5.2, y = 0.40, parse = TRUE, size = 3.5, label = expression("G" ~ ":" ~ italic(p) < 0.001)) + 
  annotate("text", x = 5.2, y = 0.30, parse = TRUE, size = 3.5, label = expression("W × G" ~ ":" ~ italic(p) == 0.597)) +
  scale_y_continuous(labels = scales::label_comma(accuracy =0.01)) + 
  labs(x = "Growing species",y = "Intrinsic growth rate",fill = NULL, tag = "a") + 
...
ggplot(field_mono_mean, aes(x = abbrev_focal, y = mean_bio)) + 
  ...
  annotate("text", x = 5.2, y = 0.30, parse = TRUE, size = 3.5, label = expression("W" ~ ":" ~ italic(p) == 0.704)) + 
  ...
  labs(x = "Growing species",y = "Competitive ability",fill = NULL, tag = "b") + 
  theme(legend.position = "none",
        legend.key = element_blank(),
...
ggplot(bio_field_mean, aes(x = abbrev_focal, y = mean_bio, color = drought2)) + 
  ...
  scale_y_continuous(labels = scales::label_comma(accuracy =0.01)) + 
  labs(x = "Growing species",y = "Cpmpetitive outcomes",fill = NULL, tag = "c") + 
  theme(legend.position = "none",
        legend.key = element_blank(),
...
Fig_S4aa/Fig_S4ba/Fig_S4ca -> Fig_S4B
