...
ggplot(df, aes(PCOA1_mean, PCOA2_mean))+
  geom_point(subset(plot_data2, drought == "D"),mapping = aes(PCOA1, PCOA2,color = abbrev), pch = 16, size = 2, alpha = 0.5) +
  geom_point(subset(plot_data2, drought == "G"),mapping = aes(PCOA1, PCOA2,color = abbrev), pch = 21, size = 2, alpha = 0.5) +
  ...
  geom_point(subset(df, drought == "D"),mapping = aes(PCOA1_mean, PCOA2_mean, color = abbrev), pch = 1, size = 3.5)+
  geom_point(subset(df, drought == "G"),mapping = aes(PCOA1_mean, PCOA2_mean, fill = abbrev, color = abbrev), pch = 21, size = 3.5)+
...
###
ggplot(df, aes(PCOA1_mean, PCOA2_mean))+
  geom_point(subset(plot_data2, drought2 == "Ambient"),mapping = aes(PCOA1, PCOA2,color = abbrev), pch = 16, size = 2, alpha = 0.8) +
  geom_point(subset(plot_data2, drought2 == "Drought"),mapping = aes(PCOA1, PCOA2,color = abbrev), pch = 21, size = 2, alpha = 0.8) +
  geom_errorbar(data = df,mapping = aes(ymax = PCOA2_mean+PCOA2_se, ymin=PCOA2_mean-PCOA2_se,color = abbrev),width=0,size=0.8,alpha = 1)+#
  geom_errorbarh(data = df,mapping = aes(xmax=PCOA1_mean+PCOA1_se,xmin=PCOA1_mean-PCOA1_se,color = abbrev),height=0,size=0.8,alpha = 1) +
  geom_point(df,mapping = aes(PCOA1_mean, PCOA2_mean,color = abbrev, shape = drought2), size = 3.5, alpha = 1) +
  #scale_color_manual(values = c("#EB7771","#1EBAB2","#A3866E","#615A8C","#EB913F","#2873A7")) +
  #scale_fill_manual(values = c("#EB7771","#1EBAB2","#A3866E","#615A8C","#EB913F","#2873A7")) +
  ...
  theme(legend.position = "right",
        legend.title = element_text(size = 11)) -> Fig_3a; Fig_3a
